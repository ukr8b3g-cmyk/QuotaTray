...
- ミニ：タイトルバーなしで週間残量、次回リセット、カウントダウンだけを表示
- コンパクト：週間残量、次回リセット、カウントダウン、最終更新だけ
- 詳細：リセット券、最近の履歴、プラン、接続状態
- 3点メニュー：ミニ/詳細/コンパクト、更新、パネル固定、最前面、位置固定、設定、Codex、ChatGPT、終了
- 設定：一般、表示、言語、通知、履歴、接続、情報
- 初期テーマはダーク、アクセントはグリーン
- 常時アニメーションを使わない
- 透明度、最前面、位置固定、位置記憶、端吸着はすべて初期OFF
- ミニ表示とミニ表示のクリック透過は初期OFF
- 設定画面自体は不透明・非最前面・位置ロック対象外
- ミニ／コンパクト／詳細は共通位置を使い、表示切替でモニターと画面端アンカーを維持する
- モニター消失時はメイン画面へ退避し、トレイ／設定から位置だけをリセット可能にする
- 情報画面にクリック可能なGitHubリポジトリリンクを表示する
