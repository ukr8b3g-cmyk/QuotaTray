...
使用分析は初期状態で無効です。設定画面で明示的に有効化した場合だけ、`CODEX_HOME\sessions` と、設定に応じて `CODEX_HOME\archived_sessions` を読み取ります。自動走査は1／5／15／30分または手動のみから選択でき、初期値は5分です。重複走査は行わず、通常は前回以降の追加分だけを読み取ります。任意のフォルダーを対象にする機能はありません。
